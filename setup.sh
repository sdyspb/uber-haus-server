...
OMV_DOMAIN="${OMV_SUBDOMAIN}.${DOMAIN}"

export DOMAIN NEXTCLOUD_DOMAIN OMV_DOMAIN
...
MODULE_NAMES=(
    ...
    "nginx + SSL certificate + subdomains"
...
while true; do
    ...
    case "$CHOICE" in
        ...
        4)
            if [[ -f "$FAQ_FILE" ]]; then
                whiptail --title "FAQ – Troubleshooting" --textbox "$FAQ_FILE" 30 100 --scrolltext
            else
                whiptail --msgbox "FAQ file not found at $FAQ_FILE" 8 60
            fi
            ;;
        5)
            break
